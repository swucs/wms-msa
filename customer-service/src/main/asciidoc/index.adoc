...
예를 들어, 잘못된 요청으로 고객를 만들려고 했을 때 다음과 같은 `400 Bad Request` 응답을 받는다.
...
고객 리소스는 고객을 등록하거나 조회할 때 사용한다.

[[resources-customer-list]]
=== 고객 목록 조회

`GET` 요청을 사용하여 서비스의 모든 고객를 조회할 수 있다.
...
`Get` 요청을 사용해서 기존 고객 하나를 조회할 수 있다.
