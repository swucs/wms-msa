...
예를 들어, 잘못된 요청으로 거래처를 만들려고 했을 때 다음과 같은 `400 Bad Request` 응답을 받는다.
...
거래처 리소스는 거래처을 등록하거나 조회할 때 사용한다.

[[resources-customer-list]]
=== 거래처 목록 조회

`GET` 요청을 사용하여 서비스의 모든 거래처를 조회할 수 있다.

operation::query-customers[snippets='response-fields,curl-request,http-response,links']

[[resources-customer-create]]
=== 거래처 등록

`POST` 요청을 사용해서 새 거래처을 등록할 수 있다.
...
`Get` 요청을 사용해서 기존 거래처 하나를 조회할 수 있다.
